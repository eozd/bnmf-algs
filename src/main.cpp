...
int main()
{
    ...
    // TODO: Improve file parsing code to allow scientific notation and multiple spaces
    ...
    while (std::getline(datafile, line)) {
        ...
        while (prev_index != std::string::npos) {
            value = std::atof(line.substr(prev_index, index - prev_index).data());
    ...
    std::tie(W, H) = bnmf_algs::nmf(X, 20, bnmf_algs::NMFVariant::Euclidean, 500);
