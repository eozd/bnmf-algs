...
TEST_CASE("Parameter checks on bld_mult_cuda", "[bld_mult_cuda]") {
    size_t x = 10, y = 5, z = 2;
    ...
    REQUIRE_NOTHROW(bld::bld_mult_cuda(X, z, model_params, 5));

    X(0, 0) = -5;
    REQUIRE_THROWS(bld::bld_mult_cuda(X, z, model_params));
...
TEST_CASE("Algorithm checks on bld_mult_cuda", "[bld_mult_cuda]") {
    size_t x = 8, y = 10, z = 3;
    shape<3> tensor_shape{x, y, z};
    SECTION("Zero matrix result is Zero") {
        ...
        tensord<3> S = bld::bld_mult_cuda(X, z, model_params);
    ...
    SECTION("Test against the results of ppmf.py implementation on "
        ...
        // =======================
        tensord<3> S = bld::bld_mult_cuda(X, z, model_params, 2000);
        ...
        // =============================
        S = bld::bld_mult_cuda(X, z, model_params, 2000, true);
